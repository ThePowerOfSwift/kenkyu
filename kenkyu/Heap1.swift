...
extension Heap1 where T: Equatable {
...
fileprivate func verifyMaxHeap(_ h: Heap1<Int>) -> Bool {
    for i in 0..<h.count {
...
fileprivate func verifyMinHeap(_ h: Heap1<Int>) -> Bool {
    for i in 0..<h.count {
        let left = h.leftChildIndex(ofIndex: i)
        let right = h.rightChildIndex(ofIndex: i)
        let parent = h.parentIndex(ofIndex: i)
